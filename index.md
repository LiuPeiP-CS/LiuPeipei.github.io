...
&emsp;&emsp;XXX终端节点预测与分析（利用已有的数据，实现对图节点的预测和图的重构）  
&emsp;&emsp;XXX基础加强项目  
&emsp;&emsp;XXX网络保护（从社工的角度分析网络保护，负责企业关系抽取与利用）  
&emsp;&emsp;零售商品分类（山东省双创竞赛机器学习组；基于word2vec+tf-idf+consine实现文本描述分类）  
&emsp;&emsp;异常声音检测（北京市科委项目；利用Qt+MFCC实现音频序列的分类）  
&emsp;&emsp;基于唇动检测的话者识别（北京市科委项目；基于网络摄像头在室内场景下捕捉到的人脸图像以及唇部特征来判别说话人）  
...
### Tech Blog
[csdn](https://blog.csdn.net/LiuPeiP_VIPL)
